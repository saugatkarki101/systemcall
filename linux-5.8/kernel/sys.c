...
{
  printk(KERN_WARNING "SAUGAT KARKI 1001644419\n");
  return 0;
}
...
{
  struct task_struct *ts = find_task_by_pid_ns(pid, &init_pid_ns);
  struct proc_stat ps;
  ...
  //copy_to_user to pass the return values from kernel space to user space

  int num_bytes = copy_to_user(ptr, &ps, sizeof(struct proc_stat));
  if(num_bytes != 0)
  {
    return EFAULT;
  }
